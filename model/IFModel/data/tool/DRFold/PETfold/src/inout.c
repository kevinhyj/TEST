...
Edouble InitPairprob(Pairprob *pp, Inside *e, Outside *f, Colprob *colprob)
{
  int p, w;
  int wl = pp->wlen;
...
void PrintPairprob(FILE *fp, Pairprob *pp, Pairprob *pp2, Alignstr *alignstr, Edouble totprob)
{
  int i, j, k, p, w;
  int wl = pp->wlen;
  ...
  for (i = 0; i < wl; i++) {
    ...
    for (j = 0; j < wl; j++) {
      ...
      p = (i > j) ? j : i;
      if (i > j)
	fprintf(fp," %10.9f", Edbl2Dbl(pp->dbl[p][w]));
      else
	fprintf(fp," %10.9f", Edbl2Dbl(pp2->dbl[p][w]));
  ...
  for (p = 0; p < wl; p++) {
    fprintf(fp," %10.9f", Edbl2Dbl(pp->sgl[p]));
